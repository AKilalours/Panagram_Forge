...
def _assessment(findings: list[Finding], detection) -> Assessment:
    ...
    if declaration is not None and declaration.status == HIGH:
        return Assessment(
            ...
            confidence=None,
            band=None,
            reason="Declared generated by the producing software.",
            detail=(
                "The file carries a generation marker: "
                + ", ".join(str(v) for v in (declaration.value or []))
                + ". A claim by the software that wrote the file, not a measurement of the "
                "pixels, and the strongest model-free indicator available. Absence of such a "
